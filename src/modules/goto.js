...
gotoModule.getMap: function(room) {
    ...
    if (!cached.costs[room.name] || (cached.age[room.name] &&
        (cached.age[room.name] + C.GOTO_MAP_CACHE) < Game.time)
    ) {
        let startCPU = Game.cpu.getUsed();
        let costs = new PathFinder.CostMatrix;
        costs = this.addTerrainToCosts(room, costs);
        cached.costs[room.name] = this.addStructuresToCosts(room, costs);
        cached.age[room.name] = Game.time;
